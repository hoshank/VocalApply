...
| `voice-native-adoption-verify.mjs` | that the page never takes `document.modelContext` away from the browser: an implementation lacking `addEventListener` is adopted and registered into, a page with none still gets the polyfill, and one injected **after load** — how an AI browser attaches its shim when its agent attaches to the tab — is adopted and re-registered into |
